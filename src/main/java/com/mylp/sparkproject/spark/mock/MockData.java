package com.mylp.sparkproject.spark.mock;
			...
			for(int j = 0; j < 10; j++) {
				...
				for(int k = 0; k < random.nextInt(100); k++) {
					...
					Row row = RowFactory.create(date, userid, sessionid, 
							...
							payCategoryIds, payProductIds, (long)random.nextInt(10));
					rows.add(row);
				}
			}
		}
...
//		}
		...
		for (long i = 0; i < 100; i++) {
			Row row = RowFactory.create(i, "product" + i,
					"{\"product_status\": " + productStatus[random.nextInt(2)] + "}");
			rows.add(row);
		}
